...
int main(int argc, char *argv[]) {
  ...
  if (!gtk_builder_add_from_file(ui_builder, "gui/main.glade", &err)) {
    g_critical("[main] failed to load ui: %s", err->message);
    g_error_free(err);
  }
  css_provider = gtk_css_provider_new();
  if (!gtk_css_provider_load_from_path(css_provider, "gui/styles.css", &err)) {
    g_critical("[main] failed to load CSS-styles: %s",
               err->message);
    g_error_free(err);
  }
  ...
  construction_buffer = GTK_TEXT_BUFFER(
      gtk_builder_get_object(ui_builder, "construction.yml_buffer"));
  loads_buffer =
      GTK_TEXT_BUFFER(gtk_builder_get_object(ui_builder, "loads.yml_buffer"));
  project_buffer =
      GTK_TEXT_BUFFER(gtk_builder_get_object(ui_builder, "project.yml_buffer"));
